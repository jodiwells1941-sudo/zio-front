...
import { getAllActiveRoundsWithTickets, ticketPurchase } from "@/app/api/lottery";
...
type Round = {
  ...
  img: string;
  prizes: Prize[];
  auth_tickets: TicketHistoryItem[];
...
const LotteryCard = ({
  ...
  return (
    ...
    <div className="lotteries-card-items bg-dark pb-2" style={{border: `1px solid ${borderColor}`}}>
      ...
      <div className="thumb" style={{ width: 100, height: 100, position: 'relative' }}>
        <Image
          fill
      ...
      <div className="content">
        <Link href={`/dashboard/lottery-details/${round?.slug}`} className="fw-bolder fs-4">
          {round?.name}
        </Link>
        <p>{drawAtText}</p>
      </div>
      ...
      {showHistory && (
        <>
          <h6 className="py-3 mt-1">
            Recent <span className="fw-7 text-warning">10</span> Tickets Purchase
          </h6>
          ...
          <div className="ticket-purchase-history-box pb-2">
            ...
            <div className="d-flex justify-content-between border-bottom py-2 border-dark-light px-2">
              ...
              <span className="value text-warning">
                {round?.winning_chance != null ? `${round.winning_chance}%` : "—"}
              </span>
            </div>
            ...
            {/* {authTickets.length > 0 ? (
              <div className="ticket-grid">
                {authTickets.map((code, idx) => (
                  <span key={idx}>
                    <i className="fa-solid fa-ticket me-1 text-warning"></i>{code}
                  </span>
                ))}
              </div>
            ) : (
              <p className="text-center text-secondary py-3">No tickets purchased yet.</p>
            )} */}
            {authTickets.length > 0 ? (
              <div className="ticket-grid">
          ...
          <div className="lotterie-btn style-3 mt-3 border-top border-secondary pt-3">
            <Link href={`/dashboard/lottery-details/${round?.slug}`}>
...
const LotterySectionTwo = ({ roundShow = 0 }: LotterySectionProps) => {
  ...
  useEffect(() => {
    ...
    const roundIds = Array.from(new Set(allRounds.map((r) => r.id).filter(Boolean)));
...
export default LotterySectionTwo;